...
// >>
//
// This is the top-level interface for the server. There is only one of these stored in the global
// TheServer`. Use this object to configure your server's settings (there are surprisingly few of 
// them.) It also contains the full server description and implementation.
//
// >>
// >>>  DISCUSSION
// >>
...
struct Server
{
	...
	//
	// IMPORTANT: Setting the advertisingName will change the system-wide name of the device. 
	// If that's not what you want, set BOTH advertisingName and advertisingShortName to as empty string 
	// ("") to prevent setting the advertising name.
	...
	//
	// According to the spec, the short name is used in case the full name doesn't fit within Extended 
	// Inquiry Response (EIR) or Advertising Data (AD).
	//
	// This is set from the constructor.
	//
	// IMPORTANT: Setting the advertisingName will change the system-wide name of the device. If that's 
	// not what you want, set BOTH advertisingName and advertisingShortName to as empty string ("") 
	// to prevent setting the advertisingname.
	...
	// This is set from the constructor.
	//
	// This is used to build the path for our Bluetooth services (and we'll go ahead and use it as the 
	// owned name as well for consistency.)
	const std::string &getServiceName() const { return serviceName; }

	// Our owned name
	//
	// D-Bus uses owned names to locate servers on the bus. Think of this as a namespace within D-Bus. 
	// We building this with the server name to keep things simple.
	...
	// serviceName: The name of our server (collectino of services)
	//
	//     This is used to build the path for our Bluetooth services. It also provides the base for 
	//     the D-Bus owned name (see getOwnedName.)    
	//
	...
	//
	//     IMPORTANT: Setting the advertisingName will change the system-wide name of the device. 
	//     If that's not what you want, set BOTH advertisingName and advertisingShortName to as 
	//     empty string ("") to prevent setting the advertising name.
	//
	//     Retrieve this value using the `getAdvertisingName()` method.
	//
	// advertisingShortName: The short name for this controller, as advertised over LE
	//
	//     According to the spec, the short name is used in case the full name doesn't fit within 
	//     Extended Inquiry Response (EIR) or Advertising Data (AD).
	//
	//     IMPORTANT: Setting the advertisingName will change the system-wide name of the device. 
	//     If that's not what you want, set BOTH advertisingName and advertisingShortName to as 
	//     empty string ("") to prevent setting the advertising name.
	...
	//
	// If the method was called, this method returns true, otherwise false.  There is no result from
...
// Extracts a VECTOR from an array of bytes ("ay")   Moverlo a UTILS???
